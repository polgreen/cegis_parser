...
void PrintVisitor::GetStringToOperatorMap() {
  ...
  String2OperatorMap["bvule"] = "<=";
  String2OperatorMap["bvugt"] = ">";
  String2OperatorMap["bvashr"] = ">>";
...
std::string PrintVisitor::ReformatLiteralString(const std::string& name) {
  ...
  if (name == "true") result = "1";
  if (name == "false") result = "0";
...
map<string, string> PrintVisitor::LogicGrammars =
    PrintVisitor::InitializeLogicGrammars();

PrintVisitor::PrintVisitor(ostream& Out, const SymbolTable* SymTbl,
                           bool Compile, bool SMTlet)
    : ASTVisitorBase("PrintVisitor"),
      IndentLevel(0),
      Out(Out),
      TheSymbolTable(SymTbl),
      SMTlet(SMTlet),
      Compile(Compile) {
  // Nothing here
}

PrintVisitor::~PrintVisitor() { Out.flush(); }
...
void PrintVisitor::VisitSynthFunCmd(const SynthFunCmd* Cmd) {
  ...
  /*  IndentLevel++;
    ...
    Cmd->GetSort()->Accept(this);
    Out << " result = 0u;" << endl;
    Out << GetIndent() << "__CPROVER_program_" << program_counter << ":;" <<
    endl; Out << GetIndent() << "return result;" << endl << "}" << endl;*/
  program_counter++;
  //  IndentLevel--;
}
...
void PrintVisitor::VisitSynthInvCmd(const SynthInvCmd* Cmd) {
  ...
  /*     string CmdName = Compile? "synth-fun" : "synth-inv";
       ...
       if (!Cmd->GetGrammarRules().empty()) {
           ...
           Out << "(StartInt Int (" << IntArgs << LogicGrammars["StartInt"] <<
     "))" << endl << endl; Out << "(ConstantInt Int (" <<
     LogicGrammars["ConstantInt"] << "))" << endl; Out << "(StartBool Bool (" <<
     BoolArgs << LogicGrammars["StartBool"] << ")))" << endl;
       }
...
void PrintVisitor::VisitVarDeclCmd(const VarDeclCmd* Cmd) {
  ...
  Out << GetIndent();
  Cmd->GetSort()->Accept(this);
  Out << " " << ReformatFunctionName(Cmd->GetName()) << ";" << endl;
  Out << "__CPROVER_havoc_object(&" << ReformatFunctionName(Cmd->GetName())
      << ");" << endl
      << endl;
}
...
void PrintVisitor::VisitInvConstraintCmd(const InvConstraintCmd* Cmd) {
  if (!Compile) {
    Out << "(inv-constraint " << Cmd->GetSynthInvName() << " "
        << Cmd->GetPreCondName() << " " << Cmd->GetTransName() << " "
...
void PrintVisitor::VisitFunTerm(const FunTerm* TheTerm) {
  if (TheTerm->GetArgs().size() == 2 &&
      (String2OperatorMap.find(TheTerm->GetFunName()) !=
           String2OperatorMap.end() ||
