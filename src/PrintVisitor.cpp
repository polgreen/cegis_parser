...
void PrintVisitor::VisitSynthFunCmd(const SynthFunCmd* Cmd) {
  ...
  /* if (!Cmd->GetGrammarRules().empty()) {
     ...
     Out << "(StartInt Int (" << IntArgs << LogicGrammars["StartInt"] << "))"
         << endl
         << endl;
     Out << "(ConstantInt Int (" << LogicGrammars["ConstantInt"] << "))" <<
   endl; Out << "(StartBool Bool (" << BoolArgs << LogicGrammars["StartBool"]
         << ")))" << endl;
   }*/
...
void PrintVisitor::VisitSynthInvCmd(const SynthInvCmd* Cmd) {
  ...
  if (!Cmd->GetGrammarRules().empty()) {
    ...
    Out << "(StartBool Bool (" << BoolArgs << LogicGrammars["StartBool"]
        << ")))" << endl;
  }
...
void PrintVisitor::VisitSetOptsCmd(const SetOptsCmd* Cmd) {
  ...
  for (auto const& Opt : Cmd->GetOpts()) {
    Out << endl
        << GetIndent() << "(" << Opt.first << " \"" << Opt.second << "\")";
  }
  Out << endl;
  IndentLevel--;
