...
bool DoesNameAndSortMatch(const SynthLib2Parser::FunDefCmd* Out, const SynthLib2Parser::SynthFunCmd* In) {
...
u32 GrammarCorrect(const SynthLib2Parser::FunDefCmd* OutFun, const SynthLib2Parser::SynthFunCmd* SynthFun, SynthLib2Parser::SymbolTable* SymTbl) {

  // find the start (= grammar axiom)
  SynthLib2Parser::NTDef* TheStart = NULL;
  ...
  if (TheStart==NULL) {
    ...
    throw SynthLib2Parser::SynthLib2ParserException((string)"Grammar for function " + FunName + " has no Start axiom.");
    return false;
  }
  
  SynthLib2Parser::Term* TheTerm = OutFun->GetTerm(); // func body
  SynthLib2Parser::GTerm* TheGrmrTerm = new SynthLib2Parser::SymbolGTerm(SynthLib2Parser::SourceLocation::None, "Start");

  SynthLib2Parser::GrammarVisitor GVstr(cout, TheGrmrTerm, SynthFun, OutFun, SymTbl);
...
int SygusTrackValidator(char* benchmark, char* track)
{
    ...
    try {
      SynthLib2Parser::CompTrack Track;
        string tr(track);
	if (tr=="CMPTRK_GENERAL") Track = SynthLib2Parser::CMPTRK_GENERAL;
        else if (tr=="CMPTRK_INV") Track = SynthLib2Parser::CMPTRK_INV;
        else if (tr=="CMPTRK_LIA") Track = SynthLib2Parser::CMPTRK_LIA;
        else if (tr=="CMPTRK_PBE") Track = SynthLib2Parser::CMPTRK_PBE;
        else throw SynthLib2Parser::SynthLib2ParserException((string)"Error: Unknown track");
        ...
        SynthLib2Parser::SymbolTable* InFileTable = ParserIn->GetSymbolTable();
        SynthLib2Parser::Program* InFileProg = ParserIn->GetProgram();

        SynthLib2Parser::InVisitor Spec(cout, InFileTable, Track);
        ...
        if (Spec.GetNumSynthCmds()==0) {
          ...
          // General Track 
          if (Track == SynthLib2Parser::CMPTRK_GENERAL) {
	  ...
	  // Example Constraints Track 
          else if (Track == SynthLib2Parser::CMPTRK_PBE) {
          ...
          else if (Track == SynthLib2Parser::CMPTRK_LIA) {
...
int SygusGrmrChecker(char* benchmark, char* solvers_out, char* smt_file)
{
	...
	ParserIn = new SynthLib2Parser::SynthLib2Parser();
        ...
        SynthLib2Parser::InVisitor Spec(cout, InFileTable,SynthLib2Parser::CMPTRK_GENERAL);
        InFileProg->Accept(&Spec);
        ...
        SynthLib2Parser::SymbolTable* OutFileTable = ParserOut->GetSymbolTable();
        SynthLib2Parser::Program* OutFileProg = ParserOut->GetProgram();

        SynthLib2Parser::OutVisitor Impl(cout, OutFileTable);
        ...
        for (u32 i = 0; i < Spec.CmdsSize(); i++) {
          ...
          if (Spec.GetCmd(i)->GetKind() != SynthLib2Parser::CMD_SYNTHFUN)
            continue;

          const SynthLib2Parser::SynthFunCmd* mystFun = dynamic_cast<SynthLib2Parser::SynthFunCmd*> (const_cast<SynthLib2Parser::ASTCmd*>(Spec.GetCmd(i)));
        ...
        if (!allAdhere || !allSolved) {
          ...
          SynthLib2Parser::PrintVisitor Pvstr(SmtFile,InFileTable,false,true);
          ...
          for (u32 i = 0; i < Spec.CmdsSize(); i++) {
            ...
            case SynthLib2Parser::CMD_FUNDEF:
            case SynthLib2Parser::CMD_SYNTHFUN:
            case SynthLib2Parser::CMD_SYNTHINV:
            case SynthLib2Parser::CMD_FUNDECL:
            case SynthLib2Parser::CMD_SORTDEF:
              Spec.GetCmd(i)->Accept(&Pvstr);
              break;

            case SynthLib2Parser::CMD_CONSTRAINT:
              {
                auto Cnstr = dynamic_cast<SynthLib2Parser::ConstraintCmd*> (const_cast<SynthLib2Parser::ASTCmd*>(Spec.GetCmd(i)));
                SmtFile << "(assert ";
            ...
            case SynthLib2Parser::CMD_SETLOGIC:
            case SynthLib2Parser::CMD_SETOPTS:
            case SynthLib2Parser::CMD_VARDECL:
            case SynthLib2Parser::CMD_PRIMEDVARDECL:
            case SynthLib2Parser::CMD_CHECKSYNTH:
            case SynthLib2Parser::CMD_INV_CONSTRAINT:
              // nothing to do
                 break;   
            }
