...
class PrintVisitor : public ASTVisitorBase {
 private:
  ...
  bool SMTlet;
  const bool Compile;  // Compile special tracks (currently invariant syntheis)
                       // to general SyGuS.
  string LogicName;
 ...
 public:
  ...
  std::set<std::string> SynthFunctions;
  std::size_t cex_counter;
  std::size_t program_counter;
  std::string declarations;
  bool first_declaration;

  PrintVisitor(ostream& Out, const SymbolTable* SymTbl = NULL,
               bool Compile = false, bool SMTlet = false);
